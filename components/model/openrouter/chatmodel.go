...
func (cm *ChatModel) Generate(ctx context.Context, in []*schema.Message, opts ...model.Option) (outMsg *schema.Message, err error) {
	...
	opt, err := cm.buildOptions(ctx, false, opts...)
	if err != nil {
		return nil, err
	}
...
func (cm *ChatModel) Stream(ctx context.Context, in []*schema.Message, opts ...model.Option) (outStream *schema.StreamReader[*schema.Message], err error) {
	ctx = callbacks.EnsureRunInfo(ctx, cm.GetType(), components.ComponentOfChatModel)
	opts, err = cm.buildOptions(ctx, true, opts...)
	if err != nil {
		return nil, err
	}
...
func (cm *ChatModel) buildOptions(_ context.Context, isStream bool, opts ...model.Option) ([]model.Option, error) {
	...
	if modelOptions.ToolChoice != nil {
		if *modelOptions.ToolChoice == schema.ToolChoiceAllowed && len(modelOptions.AllowedToolNames) > 0 {
			return nil, fmt.Errorf("tool_choice 'allowed' is not supported when allowed tool names are present")
		}

		if *modelOptions.ToolChoice == schema.ToolChoiceForced && len(modelOptions.AllowedToolNames) > 1 {
			return nil, fmt.Errorf("only one allowed tool name can be configured for tool_choice 'forced'")
		}
		
		options = append(options, model.WithToolChoice(*modelOptions.ToolChoice))
